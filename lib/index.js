...
var Surge = (function () {
  ...
  _createClass(Surge, [{
    ...
    value: function getPriceEstimateFromLatLng(cb, lat, lng) {
      ...
      uber.getPriceEstimate(lat, lng, lat, lng, function (err, response) {
        if (err || typeof response !== 'object') return cb(err);
        _this.estimates = response.prices.map(function (type) {
          var good = yay(' ✔ GOOD ');
          var bad = nay(' $ SURGE', chalk.white.bgRed(type.surge_multiplier + 'x '));
